...
F, G, H, and J are done as of this note (F was built out of order, ahead
of E, per explicit user request — both landed the same day in parallel
...
`18_succession_planning.sql` first; Phase I was built in yet another
parallel session and, as of this note, is pushed but not yet merged — check
whether it's landed before assuming its `probation-pip` section/tables
exist; Phase J's own migration is numbered 20 from this note's branch point,
which **will** collide with Phase I's `20_probation_pip.sql` if I merges
first — whichever merges second needs renumbering to 21, same pattern as
G/H); Phases I (maybe), K, L remain. If you need the original screenshots
again anyway (e.g.
...
- **Phase J — done** (built 2026-08-16). Employee Satisfaction / eNPS, a
  new module and nav group, synthetic from day one (see `20_enps.sql` and
  ...
  js/
    ...
    pages/
      ...
      data-refresh.js      "Data Refresh" — upload updated Excel workbooks,
                           ...
                           snapshot, not an accumulating table. Employee
                           Satisfaction ("18 — Employee Satisfaction")
                           bundles its 2 sheets the same way.
...
2. **Section access** — `user_access` table: `full_access` (bool, sees
   ...
   `succession`, `enps`). Each data table has a `"sectioned read"` RLS
   policy scoped to whichever sections legitimately read that table
   client-side (see the table-to-section map hardcoded in both `data.js`'s
   `SECTION_TABLES` and `06_section_based_access.sql`/`11_attendance_violations.sql`/
   ...
   `20_enps.sql` — keep these in sync if any changes).
...
19. `19_phase_g.sql` — Power BI Parity Round 2, Phase G: new `kpi_targets`
    ...
    `targetDelta()` helper.
20. `20_enps.sql` — Power BI Parity Round 2, Phase J: new `exit_surveys`/
    ...
    **Numbered 20 from this branch's own base** (the last migration on
    `main` when this was built was Phase G's 19) — if Phase I's
    `20_probation_pip.sql` merges first, this one needs renumbering to 21.
...
16. Started Power BI Parity Round 2 (a real page-by-page code+schema audit,
    ...
    instead of 18), and Phase J (Employee Satisfaction/eNPS, a new module
    and nav group — `exit_surveys`/`stage_gate_scores`, synthetic from day
    one, eNPS derived from real termination reasons and Employee Lifecycle
    Score from a reproduced early-termination signal so it has no
    dependency on Phase I's branch) — see "Power BI Parity — Round 2" above
